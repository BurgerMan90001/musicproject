...
const getRandomSongs = `-- name: GetRandomSongs :many
...
func (q *Queries) GetRandomSongs(ctx context.Context, limit int32) ([]Song, error) {
	rows, err := q.db.QueryContext(ctx, getRandomSongs, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
